...
uint32_t sx1276_get_timeonair(radio_modem_t modem, uint8_t pktLen) {
  ...
  switch( modem ) {
    case MODEM_FSK: {
    airTime = (uint32_t)((8 * (sx1276.Settings.Fsk.PreambleLen +
    ...
    case MODEM_LORA: {
      ...
      double tmp = (double)(int)((8 * pktLen - 4 * sx1276.Settings.LoRa.Datarate +
      ...
      // return us secs
      airTime = (uint32_t)( tOnAir * 1e3 + 0.999 );
    }
    break;
  }
  return airTime;
  ...
  switch( sx1276.Settings.Modem ) {
    ...
    case MODEM_LORA: {
      ...
      if((sx1276_read(REG_OPMODE) & ~RF_OPMODE_MASK) == RF_OPMODE_SLEEP) {
        ...
        while((sx1276_read(REG_OPMODE) & ~RF_OPMODE_MASK) == RF_OPMODE_SLEEP)
        {};
      }
...
void sx1276_set_rx(uint32_t timeout) {
  ...
  memset(RxTxBuffer, 0, (uint32_t) RX_BUFFER_SIZE );
  ...
  switch( sx1276.Settings.Modem ) {
    ...
    case MODEM_LORA: {
      if(sx1276.Settings.LoRa.FreqHopOn == true) {
        ...
        // DIO0=TxDone
        sx1276_write(REG_DIOMAPPING1, RFLR_DIOMAPPING1_DIO0_01);
      }
    }
    break;
  }
...
void Port_8 (void) {
    ...
    GPIO_clearInterrupt(GPIO_PORT_P8, 0xFF);
}
